...
User.findOne({ where: { name: "jared" } }).then(user => {
  Product.findOne({ where: { name: "black shirt" } }).then(product => {
    user
      .createReview({
        rating: 5,
        comment: "not the best, but still functions as a shirt",
        author: user.name
      })
      .then(review => {
...
User.findOne({ where: { name: "ivan" } }).then(user => {
  ...
  Product.findOne({ where: { name: "red pants" } }).then(product => {
    user
      .createReview({ rating: 10, comment: "like a glove!", author: user.name })
      .then(review => {
        product.addReview(review);
      });
  });
  Product.findOne({ where: { name: "black dress" } }).then(product => {
    user
      .createReview({
        rating: 1,
        comment: "wtf i ordered the black shirt",
        author: user.name
      })
      .then(review => {
        product.addReview(review);
...
User.findOne({ where: { name: "gonza" } }).then(user => {
  Product.findOne({ where: { name: "black shirt" } }).then(product => {
    user
      .createReview({ rating: 5, comment: "eh...whatever", author: user.name })
      .then(review => {
        product.addReview(review);
      });
  });
  Product.findOne({ where: { name: "blue shirt" } }).then(product => {
    user
      .createReview({ rating: 5, comment: "whatever....", author: user.name })
      .then(review => {
        product.addReview(review);
      });
  });
...
User.findOne({ where: { name: "milo" } }).then(user => {
  ...
  Product.findOne({ where: { name: "blue shirt" } }).then(product => {
    user
      .createReview({
        rating: 10,
        comment: "yaaaa, i wear this everyday",
        author: user.name
      })
      .then(review => {
        product.addReview(review);
  ...
  Product.findOne({ where: { name: "red pants" } }).then(product => {
    user
      .createReview({
        rating: 8,
        comment: "cant go wrong with red",
        author: user.name
      })
      .then(review => {
        product.addReview(review);
  ...
  Product.findOne({ where: { name: "black dress" } }).then(product => {
    user
      .createReview({ rating: 1, comment: "no thx", author: user.name })
      .then(review => {
        product.addReview(review);
      });
  });
...
User.findOne({ where: { name: "nati" } }).then(user => {
  Product.findOne({ where: { name: "black shirt" } }).then(product => {
    user
      .createReview({ rating: 10, comment: "good shirt", author: user.name })
      .then(review => {
        product.addReview(review);
      });
  });
  Product.findOne({ where: { name: "blue shirt" } }).then(product => {
    user
      .createReview({ rating: 10, comment: "great shirt", author: user.name })
      .then(review => {
        product.addReview(review);
      });
  });
  Product.findOne({ where: { name: "red pants" } }).then(product => {
    user
      .createReview({ rating: 5, comment: "more red plz..", author: user.name })
      .then(review => {
        product.addReview(review);
      });
  });
  Product.findOne({ where: { name: "black dress" } }).then(product => {
    user
      .createReview({
        rating: 6,
        comment: "ok, but not black enough",
        author: user.name
      })
      .then(review => {
        product.addReview(review);
...
User.findOne({ where: { name: "jared" } }).then(user => {
  Product.findAll().then(products => {
    ...
    user
      .createPastOrder({
        name: new Date("November 10, 2008 03:24:00"),
        state: "completado"
      })
      .then(pastOrder => {
    ...
    user
      .createPastOrder({
        name: new Date("June 23, 2018 03:24:00"),
        state: "completado"
      })
      .then(pastOrder => {
...
User.findOne({ where: { name: "nati" } }).then(user => {
  Product.findAll().then(products => {
    user
      .createPastOrder({ name: new Date(), state: "completado" })
      .then(pastOrder => {
        pastOrder
          .addProducts([products[7], products[3], products[1]])
          .then(() => {
            Product_cart.findAll({ where: { cartId: pastOrder.id } }).then(
              quantities => {
    ...
    user
      .createPastOrder({
        name: new Date("October 1, 1999 03:24:00"),
        state: "completado"
      })
      .then(pastOrder => {
    ...
    user
      .createPastOrder({
        name: new Date("May 15, 2019 03:24:00"),
        state: "completado"
      })
      .then(pastOrder => {
